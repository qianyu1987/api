...
import { createApiKey, createInviteCode, hashApiKey, keyPrefix, encryptSecret, decryptSecret } from '../lib/crypto.js'
...
export class AuthService {
  ...
  async register(username: string, password: string, inviteCode?: string): Promise<PublicUser> {
    ...
    return this.db.tx(async (client) => {
      ...
      if (inviteCode?.trim()) {
        inviter = await one<any>(client, `SELECT id, invite_code FROM users WHERE invite_code = $1 AND disabled_at IS NULL AND status = 'active'`, [inviteCode.trim().toUpperCase()])
      ...
      for (let attempt = 0; attempt < 10; attempt += 1) {
        try {
          const user = await one<any>(client,
            `INSERT INTO users(username, password_hash, invite_code, invited_by)
             VALUES ($1, $2, $3, $4)
             RETURNING id, username, role, invite_code, created_at`,
            [cleanUsername, passwordHash, code, inviter?.id ?? null],
          )
  ...
  async login(username: string, password: string): Promise<PublicUser> {
    const row = await this.db.one<any>('SELECT id, username, password_hash, role, invite_code, created_at, disabled_at, status FROM users WHERE lower(username) = $1', [username.trim().toLowerCase()])
  ...
  async listApiKeys(userId: string): Promise<any[]> {
    const rows = await this.db.query<any>(`SELECT id, name, key_prefix, created_at, last_used_at, revoked_at FROM api_keys WHERE user_id = $1 ORDER BY created_at DESC`, [userId])
    return rows.map((row) => ({ id: String(row.id), name: row.name, prefix: row.key_prefix, createdAt: new Date(row.created_at).toISOString(), lastUsedAt: row.last_used_at ? new Date(row.last_used_at).toISOString() : null, revoked: Boolean(row.revoked_at) }))
  }
  ...
  async authenticateApiKey(rawKey: string): Promise<{ user: PublicUser; key: { id: string; name: string }; }> {
    const row = await this.db.one<any>(
      `SELECT k.id AS key_id, k.name AS key_name, u.id, u.username, u.role, u.invite_code, u.created_at
