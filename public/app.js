(() => {
  ...
  function renderAdmin(tab, data) {
    ...
    if (tab === 'media-admin') {
      ...
      return '<p class="notice">目标利润率 30%，已考虑充值倍率、支付费用和返利。常规成本决定售价，实际成本单独记录。金额单位：人民币元 / 张；视频为元 / 秒。排队状态仅显示平台安全摘要，不展示上游编号或原始错误。</p>' + prices.map(p => form('media-price', [
      ...
      ],'保存媒体价格')).join('') + '<div class="section-heading compact-heading"><div><h4>作品管理</h4><p class="muted">新作品默认不公开。发布前可填写标题，精选作品优先展示；移出广场不会删除账单和生成记录。</p></div></div>' + table(['预览','用户 / 时间','类型 / 状态','排队 / 接单','广场标题','公开状态','操作'],mediaRows,'暂无媒体作品')
    }
    if (tab === 'channels') {
      ...
      const balance = item => { const b=item.upstreamBalance||{}; if(b.status==='available'){const scope=b.scope==='api_key'?'<small>API Key 独立配额（非账户钱包）</small>':'';const overdraft=b.overdraft!=null?'<small>Key 超额使用 '+esc((b.unit||'¥')+' '+Number(b.overdraft).toLocaleString('zh-CN',{maximumFractionDigits:8}))+'</small>':'';return '<div class="upstream-balance"><strong>'+esc((b.unit||'¥')+' '+Number(b.remaining).toLocaleString('zh-CN',{maximumFractionDigits:8}))+'</strong>'+scope+(b.quota!=null?'<small>Key 配额 '+esc(Number(b.quota).toLocaleString('zh-CN',{maximumFractionDigits:8}))+'</small>':'')+overdraft+(b.message?'<small>'+esc(b.message)+'</small>':'')+'<small>'+date(b.checkedAt)+' 查询</small></div>'}return '<div class="upstream-balance unavailable"><strong>'+esc(b.status==='error'?'查询失败':'不可查询')+'</strong><small>'+esc(b.message||'供应商未提供余额接口')+'</small><small>'+date(b.checkedAt)+' 检查</small></div>' }
      ...
      return editor + '<div class="admin-toolbar"><p>额度每 60 秒自动更新；RIPP / YYAPI 当前接口返回的是 API Key 独立配额，不是账户钱包余额；点击可立即重新查询上游。</p><button class="button secondary" type="button" data-refresh-channel-balances>刷新上游额度</button></div>' + table(['渠道', '上游额度（Key）', '优先级', '超时', '模型映射', '状态', '操作'], rows, '尚未添加上游渠道')
    }
    if (tab === 'channel-costs') {
      ...
      const editor = form('channel-cost', [
        ...
        field('highContextIncreasePercent', '超过 272K 的上游成本涨幅（%）', '20', 'number', 'min="0" max="1000" step="0.01" required'),
        field('priceSource', '成本来源（账单日期或供应商报价）', '', 'text', 'maxlength="512" required'),
        '<p class="admin-note wide">这里仅记录上游在长上下文下的实际成本倍率；用户售价始终使用标准价格。填写已包含账户折扣的人民币实际成本，不再乘汇率。缓存免费时明确填 0。保存后立即用于新请求，已有账单及用户售价保持原快照。</p>',
      ...
      return editor + '<p class="admin-section-title">当前上游成本 · 输入 / 输出 / 缓存（元 / 百万 Token）</p>' + table(['渠道', '公开模型', '标准成本', '272K+ 上游成本倍率', '来源', '生效时间'], rows, '尚未配置渠道成本') + '<p class="admin-section-title">最近 50 次修改</p>' + table(['时间', '修改人', '渠道 / 模型', '修改前', '修改后', '来源'], audits, '暂无成本修改记录')
    }
    if (tab === 'prices') {
      ...
      const editor = form('price', [field('modelPattern', '模型匹配', '*', 'text', 'required'), field('marginBps', '目标毛利率（基点）', '8000', 'number', 'min="0" max="9999" data-margin'), ...inputs, check('active', '启用'), '<p class="admin-note wide">所有上下文长度统一使用标准用户售价；上游超过 272K 的成本倍率只在“渠道成本”中维护，不改变用户售价。售价按目标毛利率自动计算；数据库仍以微元/百万 Token 保存。</p>'], '保存模型价格')
      const rows = items.map((item) => '<tr><td><strong>' + esc(item.model_pattern) + '</strong><small class="subline">' + esc(item.price_source || '手工设置') + '</small></td><td>' + money({ micros: rate(item, 'input_sell') }) + '<small class="subline">成本 ' + money({ micros: rate(item, 'input_cost') }) + ' · ' + margin(rate(item, 'input_cost'), rate(item, 'input_sell')) + '</small></td><td>' + money({ micros: rate(item, 'output_sell') }) + '<small class="subline">成本 ' + money({ micros: rate(item, 'output_cost') }) + ' · ' + margin(rate(item, 'output_cost'), rate(item, 'output_sell')) + '</small></td><td>' + money({ micros: rate(item, 'cache_sell') }) + '</td><td><span class="state ' + (item.active ? 'good' : 'bad') + '">' + (item.active ? '启用' : '停用') + '</span></td><td class="admin-action-cell"><button class="small-button admin-edit" type="button" data-kind="price" data-item="' + esc(JSON.stringify(item)) + '">编辑</button><button class="small-button danger-button admin-delete" type="button" data-kind="price" data-id="' + esc(item.id) + '">停用</button></td></tr>')
      return '<div class="admin-toolbar"><p>只会初始化已启用渠道实际映射的模型；上游页面标注为美元的数值按人民币 1:1 结算，不再乘以 7.2。用户售价不按 272K 分层。</p><button class="button secondary" type="button" data-bootstrap="openai-prices">按上游 1:1 价格初始化</button></div>' + editor + table(['模型 / 来源', '输入标准售价', '输出标准售价', '缓存标准售价', '状态', '操作'], rows, '尚未配置模型价格')
    }
    if (tab === 'fixed-prices') {
    ...
    if (tab === 'users') {
      return table(['用户', '历史总充值（实付）', '累计到账额度', '可用余额', 'Token 折扣', '本周期额度', '到期 / 重置', '状态', '操作'], items.map((item) => {
        ...
        const historyPaid = item.total_paid_micros ?? item.total_topup_paid_micros ?? '0'
        return '<tr><td><strong>' + esc(item.username) + '</strong><small class="subline">' + esc(item.email || '未验证邮箱') + '</small></td><td><strong>' + money({ micros: historyPaid }) + '</strong><small class="subline">钱包充值 ' + money({ micros: item.total_topup_paid_micros || '0' }) + ' · 套餐已计入</small></td><td>' + money({ micros: item.total_topup_credit_micros }) + '<small class="subline">钱包实际到账额度</small></td><td>' + money({ micros: wallet - walletReserved }) + '<small class="subline">钱包 ' + money({ micros: wallet }) + ' · 冻结 ' + money({ micros: walletReserved }) + '</small></td><td><form class="inline-discount" data-user-id="' + esc(item.id) + '"><div class="discount-input"><input name="discount" type="number" min="0" max="99" step="1" value="' + esc(discount) + '" aria-label="' + esc(item.username) + ' 折扣减免比例"><span>%</span></div><button class="small-button" type="submit">保存</button><small class="subline">实际支付 ' + (100 - discount) + '%</small></form></td><td>' + (item.plan_status === 'active' ? planProgressMarkup({ planQuotaMicros: planQuota, planUsedMicros: planUsed, planReservedMicros: planReserved, planBookRemainingMicros: planBook, planRemainingMicros: planAvailable, planStatus: item.plan_status }, true) : '—') + '</td><td>' + (item.plan_status === 'active' ? date(item.plan_expires_at) + '<small class="subline">下次 ' + date(item.plan_next_reset_at) + '</small>' : '—') + '</td><td><span class="state ' + (item.status === 'active' ? 'good' : 'bad') + '">' + esc(item.status) + '</span></td><td><div class="admin-user-actions"><button class="small-button wallet-adjust" type="button" data-id="' + esc(item.id) + '" data-username="' + esc(item.username) + '">充值/扣费</button>' + (item.plan_status === 'active' ? '<button class="small-button admin-reset-plan" type="button" data-id="' + esc(item.id) + '">重置额度</button>' : '') + '</div></td></tr>'
      }))
    }
  ...
  $('#media-tasks').addEventListener('click',event=>{
    const cancel=event.target.closest('[data-media-cancel]')
    if(cancel&&!mediaBusy&&!mediaUploading){
      mediaBusy=true;pending(cancel,true,'取消中…')
      api('/api/me/media/tasks/'+encodeURIComponent(cancel.dataset.mediaCancel),{method:'DELETE'}).then(()=>{toast('已取消排队，额度已退回');return loadMedia()}).catch(error=>toast(error.message,true)).finally(()=>{mediaBusy=false;pending(cancel,false)})
      return
    }
    const button=event.target.closest('[data-media-retry]');if(!button||mediaBusy||mediaUploading)return
    const task=mediaTaskInputs.get(button.dataset.mediaRetry);if(!task?.input)return
    const input=task.input,f=$('#media-form')
    const setRadio=(name,value)=>{const node=[...f.elements[name]].find(item=>item.value===String(value));if(node)node.checked=true}
    setRadio('kind',input.kind||task.kind);if(input.engine)setRadio('engine',input.engine);updateMediaControls()
    setRadio('size',input.size||'1K');setRadio('ratio',input.ratio||'16:9');if(input.mode)setRadio('mode',input.mode)
    f.elements.prompt.value=input.prompt||'';if(f.elements.seconds&&input.seconds)f.elements.seconds.value=input.seconds
    for(const name of ['images','audios'])if(f.elements[name])f.elements[name].value=Array.isArray(input[name])?input[name].join('\n'):''
    for(const name of ['first_frame','last_frame'])if(f.elements[name])f.elements[name].value=input[name]||''
    $$('[data-upload-list]').forEach(node=>{node.innerHTML=''})
    originalPrompt=null;$('#media-restore').classList.add('hidden');$('#media-expand-status').textContent=''
    show('media');f.scrollIntoView({behavior:'smooth',block:'start'});f.elements.prompt.focus();scheduleMediaQuote()
    toast(task.assetsMayHaveExpired?'原素材地址可能已失效，请重新上传后再生成':'已填入原参数，请确认新报价后提交')
  })
