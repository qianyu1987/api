(() => {
  ...
  function renderAdmin(tab, data) {
    ...
    if (tab === 'users') {
      return table(['用户', '累计充值额度', '可用余额', 'Token 折扣', '本周期额度', '到期 / 重置', '状态', '操作'], items.map((item) => {
        ...
        return '<tr><td><strong>' + esc(item.username) + '</strong><small class="subline">' + esc(item.email || '未验证邮箱') + '</small></td><td>' + money({ micros: item.total_topup_credit_micros }) + '<small class="subline" title="已支付的钱包充值与套餐购买金额合计">实付总额 ' + money({ micros: item.total_paid_micros }) + '</small></td><td>' + money({ micros: wallet - walletReserved }) + '<small class="subline">钱包 ' + money({ micros: wallet }) + ' · 冻结 ' + money({ micros: walletReserved }) + '</small></td><td><form class="inline-discount" data-user-id="' + esc(item.id) + '"><div class="discount-input"><input name="discount" type="number" min="0" max="99" step="1" value="' + esc(discount) + '" aria-label="' + esc(item.username) + ' 折扣减免比例"><span>%</span></div><button class="small-button" type="submit">保存</button><small class="subline">实际支付 ' + (100 - discount) + '%</small></form></td><td>' + (item.plan_status === 'active' ? planProgressMarkup({ planQuotaMicros: planQuota, planUsedMicros: planUsed, planReservedMicros: planReserved, planBookRemainingMicros: planBook, planRemainingMicros: planAvailable, planStatus: item.plan_status }, true) : '—') + '</td><td>' + (item.plan_status === 'active' ? date(item.plan_expires_at) + '<small class="subline">下次 ' + date(item.plan_next_reset_at) + '</small>' : '—') + '</td><td><span class="state ' + (item.status === 'active' ? 'good' : 'bad') + '">' + esc(item.status) + '</span></td><td><div class="admin-user-actions"><button class="small-button wallet-adjust" type="button" data-id="' + esc(item.id) + '" data-username="' + esc(item.username) + '">充值/扣费</button>' + (item.plan_status === 'active' ? '<button class="small-button admin-reset-plan" type="button" data-id="' + esc(item.id) + '">重置额度</button>' : '') + '</div></td></tr>'
      }))
    }
  ...
  async function loadMedia() {
    ...
    try {
      ...
      const labels={queued:'排队中',submitting:'提交中',processing:'生成中',unknown:'自动确认中',completed:'已完成',failed:'失败，额度已自动退回'}
      $('#media-tasks').innerHTML=table(['类型 / 时间','进度','钱包额度','结果','操作'],tasks.items.map(t=>'<tr><td>'+(t.kind==='video'?'视频生成':'图片生成')+'<small class="subline">'+date(t.createdAt)+'</small></td><td>'+esc(labels[t.status])+' '+Number(t.progress)+'%</td><td>'+money({micros:t.chargeMicros})+' · '+(t.gift?'免费福利 · '+(t.reserved?'使用中':t.status==='completed'?'已使用':'已退回'):t.charged?'已扣费':t.reserved?'冻结中':t.refundStatus==='returned'?'已自动退回':'已释放')+'</td><td>'+(t.resultUrl?'<a href="'+esc(t.resultUrl)+'" target="_blank" rel="noopener noreferrer">查看 / 下载作品</a>':esc(t.error||'等待生成'))+'</td><td>'+(t.canRetry?'<button class="small-button" type="button" data-media-retry="'+esc(t.id)+'">'+(t.status==='completed'?'再次创作':'重新生成')+'</button>':'—')+'</td></tr>'))
