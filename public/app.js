(() => {
  ...
  function renderAdmin(tab, data) {
    ...
    if (tab === 'prices') {
      ...
      const editor = form('price', [field('modelPattern', '模型匹配', '*', 'text', 'required'), field('marginBps', '目标毛利率（基点）', '8000', 'number', 'min="0" max="9999" data-margin'), ...inputs, check('active', '启用'), '<p class="admin-note wide">固定汇率：1 USD = ¥7.20。售价按目标毛利率自动计算；数据库仍以微元/百万 Token 保存。</p>'], '保存模型价格')
