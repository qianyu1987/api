...
任务 `queued` 表示平台正在等待可用的生成资源；视频遇到上游队列满、
`429` 或明确的 `503` 未接单时会自动退避并尝试兼容渠道。客户端可使用返回的
`queueStatus`（`waiting`、`switching`、`accepted`）展示“等待生成资源”“正在切换
生成服务”或“视频生成中”，不需要重复提交。平台最多等待 30 分钟，期间只冻结一次
额度；超时后任务以 `failed` 结束并全额释放预扣。

任务 `unknown` 表示提交或查询结果暂时无法确认。若已经有上游任务编号，平台只会
继续查询原任务，不会重复提交；视频的确认窗口为 45 分钟，图片仍为 1 分钟。
确认窗口结束后自动退回额度。提交结果明确成功后进入 `processing`，不会再切换渠道。
已失败任务的用户文案不会包含上游原始错误。

排队期间可以调用 `DELETE /v1/media/tasks/{id}` 取消并立即释放冻结额度；已经接单的
任务不允许取消。任务返回 `canCancel`、`queuedAt`、`nextRetryAt`、`refundStatus`，
不返回渠道编号、上游任务编号或成本快照。已完成任务提供安全输入供“再次创作”，
失败任务可“重新生成”，新提交必须使用新的幂等编号并重新报价。
